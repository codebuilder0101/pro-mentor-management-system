...
type Props = {
  depoimentos: ArtigoRow[];
};

export default function MentorshipProgramClient({ depoimentos }: Props) {
  ...
  const phases = [
    {
      ...
      sessions: 4,
      focus:
        'Diagnóstico Inicial, Contrato de Jornada, Visão de Futuro e Diagnóstico Psicológico-Operacional, Direção - Âncoras de Carreira, Visualização Estratégica,  Base Pessoal - Valores, Energia, Crenças e Sacrifícios',
    },
    {
      n: 2,
    ...
    },
    {
      ...
      sessions: 4,
      focus:
        'Influência, Carisma, Conexões, Confiança, Reputação e Promoção Estratégica, Gestão de Pessoas, Performance, Carreira Y, Escolhas e Poder Profissional',
    },
    {
      n: 4,
      ...
      sessions: 3,
      focus:
        'Estratégia, Informação, Resultado com Consciência,  Excelência, Capricho, Comunicação Executiva, Liderança Aplicada e Caráter',
    },
    {
      n: 5,
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <section className="py-20 bg-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <p className="text-center text-gray-600 mb-12 max-w-2xl mx-auto">
            Visão simplificada do percurso. A distribuição exata das 16 sessões entre fases e formatos
            (individual ou grupo) é alinhada na proposta de mentoria.
          </p>
      ...
      {depoimentos.length > 0 ? (
        <section className="py-20 bg-white border-t border-gray-100">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <h2 className="text-3xl md:text-4xl font-bold text-center mb-3 text-gray-900">Depoimentos</h2>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
              {depoimentos.map((d) => (
                <Card
                  key={d.id}
                  className="h-full flex flex-col border border-gray-100 bg-white shadow-md rounded-xl"
                >
                  <p className="text-gray-700 text-[15px] md:text-base leading-relaxed flex-1 mb-6 text-justify grow">
              ...
              >
                <Card>
                  <div className="flex justify-between items-center">
                    ...
                    <span className="text-2xl text-[#2563EB] shrink-0">{activeFaq === index ? '−' : '+'}</span>
                  </div>
                  {activeFaq === index && (
                    <p className="mt-4 mx-4 mx-auto text-gray-600 text-justify">{faq.answer}</p>
                  )}
                </Card>
              </div>
            ))}
