...
export default function ScheduleSessionClient({ defaultEmail }: Props) {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      const ok = res.ok && data.ok === true && typeof data.id === 'string' && data.id.length > 0;
        ...
        <div className="max-w-lg w-full mx-auto">
          <Card className="text-center border border-gray-100 shadow-lg shadow-gray-200/50 overflow-hidden !p-0">
            ...
            <div className="px-8 py-8 space-y-5 text-left text-[15px] leading-relaxed text-gray-600">
              ...
              <p>
                O pedido foi registrado na plataforma. A equipe confirmará data e horário e entrará em contato pelo
                email <strong className="font-semibold text-gray-900">{formData.email}</strong> ou pelo telefone{' '}
                <strong className="font-semibold text-gray-900">{formData.phone}</strong>, se necessário.
              </p>
              {formData.mentorshipModel === 'online' ? (
                <p className="text-gray-600">
                  Para sessões online, combinamos o link da videoconferência após a confirmação do horário.
                </p>
              ) : (
                <p className="text-gray-600">
                  Formato <strong className="font-semibold text-gray-900">presencial</strong> — combinaremos o local ou
                  os próximos passos pelo telefone indicado.
                </p>
              )}
            </div>
            ...
            <form onSubmit={handleSubmit} className="px-6 py-8 md:px-10 md:py-10 space-y-6">
              ...
              <div className="border-t border-gray-100 pt-8">
                ...
                <p className={`${hintClass} mt-3 text-center text-xs`}>
                  Ao enviar, você concorda que a equipa utilize os dados para contacto sobre este pedido.
                </p>
              </div>
            </form>
          </Card>
